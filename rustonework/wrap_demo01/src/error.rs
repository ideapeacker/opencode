...
use failure::Error as PasetoTokenError;
...
pub async fn return_error(r: Rejection) -> Result<impl Reply, Rejection> {
    if let Some(Error::DatabaseQueryError(e)) = r.find() {
        ...
        event!(Level::ERROR, "Cann't decrypt token");
        Ok(warp::reply::with_status(
            "Cann't decrypt token".to_string(),
            StatusCode::UNAUTHORIZED,
        ))
    } else if let Some(Error::TokenError(e)) = r.find() {
        event!(Level::ERROR, "TokenError:{:?}", e);
        Ok(warp::reply::with_status(
            e.to_string(),
            StatusCode::UNAUTHORIZED,
        ))
    ...
    } else if let Some(error) = r.find::<PasetoTokenError>() {
        event!(Level::ERROR, "{}", error);
        Ok(warp::reply::with_status(
            error.to_string(),
            StatusCode::UNAUTHORIZED,
