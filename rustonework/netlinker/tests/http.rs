...
use std::{println, string::String, thread, time::Duration};
...
async fn http_proxy() {
    ...
    //     c.write_all(
    //         b"GET http://www.baidu.com/ HTTP/1.0\r\nHost: www.baidu.com\r\nAccept: */*\r\n\r\n",
    //     )
    //     .await
    //     .unwrap();
    //     c.flush().await.unwrap();

    //     println!("{:?}", c.local_addr().unwrap());
    ...
    {
        let mut c = TcpStream::connect("127.0.0.1:5110").await.unwrap();
        c.write_all(b"CONNECT https://www.baidu.com/ HTTP/1.0\r\n\r\n")
            .await
            .unwrap();
        c.flush().await.unwrap();
